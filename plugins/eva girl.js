...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to bot voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('bot') && conf.TALKING_bot !== 'true') {        
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_bot == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel bot özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'bot yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'bot yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'bot, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'bot, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional bot features. Turn your account into a ai chatbot!'
    already_on = 'bot artificial intelligence is already fully functional.'
    already_off = 'bot artificial intelligence is currently running semi-functional.'
    succ_on = 'bot Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'bot Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ bot സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'bot കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'bot AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'bot പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി bot സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'bot ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.bot on / off' }, (async (message, match) => {
    var bot_status = `${conf.TALKING_bot}`
    if (match[1] == 'on') {
        if (bot_status == 'true') {
