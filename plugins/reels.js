...
Asena.addCommand({pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram" , dontAddCommandList: true }, async (message, match) => {
   ...
   const { status, type, data } = await instagram(match[1], 'SuperSmart')
    if (!status) return await message.sendMessage('✅️Example : https://www.instagram.com/p/CBACL5vAlL3/?utm_medium=copy_link\n\n*Change /reel/ to /p/ Then give Command .insta*\n\n*Any doubt ask to Safeer*')

    reply = await message.client.sendMessage(message.jid,UPLOAD_ING , MessageType.text, { quoted: message.data });
    
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "*Codded by SuperSmart*", quoted: message.data })
    
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "*Codded by SuperSmart*", quoted: message.data })
